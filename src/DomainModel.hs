...
instance FromJSON VoucherList where
  parseJSON = withObject "VoucherList" $ \l -> VoucherList
    <$> l .: "content"
    <*> l .: "totalPages"
    <*> l .: "size"  
...
instance FromJSON Voucher where
  parseJSON = withObject "Voucher" $ \v -> Voucher
    <$> v .: "id"
    <*> v .: "voucherNumber"
    <*> v .: "voucherDate"
    <*> v .: "contactName"
...
buildFlatItem item = FlatLineItem
...
buildDenormalizedItem (voucher,item) = DenormalizedItem
